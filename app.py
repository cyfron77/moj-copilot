...
def dodaj_wskazniki(dane: pd.DataFrame) -> pd.DataFrame:
    """Dodaje SMA, RSI, Bollinger Bands, MACD i ATR do danych OHLC."""
    if dane is None or dane.empty:
        return dane

    df = dane.copy()

    # Upewnij się, że mamy standardowe nazwy kolumn
    # (ważne przy multi-tickerach z yfinance)
    kolumny = {c: c.title() for c in df.columns}
    df.rename(columns=kolumny, inplace=True)
...
def pobierz_dane_multi(lista_symboli, period="3mo", interval="1d") -> dict:
    ...
    try:
        ...
        if isinstance(raw.columns, pd.MultiIndex):
            # group_by="ticker" -> pierwszym poziomem jest ticker
...
# ---------------------------------------------------------
# AI silnik werdyktu (parametryzowany wagami)
# ---------------------------------------------------------
...
def oblicz_ai_werdykt(
    ...
    # Korekta za jakość danych
    if data_len < 30:
        base_score *= 0.5
        jakosc_flags.append("⚠️ Mało danych (mniej niż 30 świec) – score obniżony x0.5")
    ...
    if base_score >= 3:
        ...
        kolor = "success"
        komentarz = "Przewaga sygnałów prowzrostowych. Rozważ szukanie wejścia zgodnie z własną strategią."
    elif base_score <= -3:
        ...
        kolor = "info"
        komentarz = "Sygnały są mieszane lub rynek w konsolidacji. Wstrzymaj się z agresywną decyzją."
...
# Ustawienia AI werdyktu – wagi sygnałów
st.sidebar.header("🤖 Ustawienia AI werdyktu")
...
w_sent = st.sidebar.slider(
    "Waga sentymentu newsów:",
    min_value=0.0,
    max_value=3.0,
    value=1.0,
    step=0.5,
...
c6.metric("AI Score", f"{werdykt['score']:.2f}", werdykt["status"])
...
komunikat_werdyktu = (
    f"🎯 **WERDYKT AI COPILOTA: {werdykt['status']}**\n\n"
    f"- {werdykt['trend_opis']} | {werdykt['rsi_opis']} | {werdykt['macd_opis']} | "
    f"Sentyment: {werdykt['sent_opis']}\n"
    f"- *{werdykt['komentarz']}*\n"
...
with tab3:
    ...
    if roznica_sl > 0:
        ...
        if instrument_typ == "CFD XTB":
            ...
            margin_needed = wartosc_pozycji  # dla akcji/ETF margin = wartość pozycji
        ...
        st.success(
            ...
            f"- Szacowana strata przy SL: **{ryzyko_na_sl:,.2f}** (w jednostkach kapitału)\n"
